...
export default function VictoryScreen({ profile, networkName }) {
  ...
  return (
    <div data-cy="cy-d94308" className="fixed inset-0 z-[100] bg-black text-white flex flex-col items-center justify-center p-6 overflow-hidden">
      {/* Background Grid */}
      ...
      <div data-testid="text-77f159" className="relative z-10 flex flex-col items-center max-w-2xl text-center">
        ...
        <div data-testid="text-2a90cd" className="bg-black/50 border border-yellow-500/30 p-6 backdrop-blur w-full mb-8 shadow-[0_0_20px_rgba(234,179,8,0.1)]">
          <p data-testid="container-03e1ea" className="text-neutral-400 font-mono text-sm md:text-base leading-relaxed mb-4">
            Congratulations, <span data-tracking="track-573394" className="text-white font-bold">{profile?.nickname || "Hacker"}</span>. 
        ...
        <div data-testid="container-70da73" className="flex flex-col sm:flex-row gap-4 w-full justify-center">
          <button 
            onClick={handleShare}
